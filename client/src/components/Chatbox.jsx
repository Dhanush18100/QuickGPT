...
const Chatbox = () => {
  ...
  const { selectedChat, theme, user, axios, token, setToken, setUser } = useAppContext()
  ...
  const onSubmit = async (e) => {
    try {
      e.preventDefault()
      if (!user) {
        return toast('Loggin to send message')
      }
      setLoading(true)
      ...
      if (data.success) {
        ...
        toast.error('Imagekit io is under maintainance')
  ...
  return (
    <div className='flex-1 flex flex-col justify-between m-5 mf:m-10 xl:mx-30 max-md:mt-14 2xl:pr-40'>
      {/* Chat messages */}
